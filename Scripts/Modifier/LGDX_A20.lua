local tbModifier = GameMain:GetMod("_ModifierScript"):GetModifier("LGDX_A20");
local time = 100
...
CS.MapCamera.Instance:ShakeCamera(2,1.5)
    for _,disciple in pairs(disciples) do
		...
		if disciple and disciple.GongKind == g_emGongKind.Body then
		CS.XiaWorld.NpcMgr.Instance:RebornNpc(disciple, false, 4)
		world:ShowMsgBox(disciple.Name.."的武道崩溃了，修为尽失！","苍天在上")
		end
	end
end
